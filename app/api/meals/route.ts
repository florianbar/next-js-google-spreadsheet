...
export async function GET() {
    try {
        ...
        const response: GaxiosResponse<sheets_v4.Schema$ValueRange> = await googleSheets.spreadsheets.values.get({
            auth,
...
export async function POST(request: Request) {
    try {
        ...
        const { auth, googleSheets } = await getAuth();
        const date = requestBody.date ?? getTodayISOString();
        ...
        const response = await googleSheets.spreadsheets.values.append({
            ...
            resource: {
                values: [
                    [date, requestBody.food, requestBody.quantity, requestBody.healthy],
                ],
            },
        });
